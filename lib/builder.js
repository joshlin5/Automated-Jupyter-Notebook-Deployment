...
const isoDir = path.join( path.dirname(require.main.filename), 'syslinux' );
...
class Builder {
	...
	await dockerProvider.run("p:latest", `-v${scriptsDir}:/scripts -v${isoDir}:/syslinux --privileged`, "/scripts/package-iso.sh")
